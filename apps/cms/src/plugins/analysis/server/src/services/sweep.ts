...
export const sweep = ({ strapi }: { strapi: Core.Strapi }) => ({
  async run() {
    const notifySlack = () => strapi.plugin('notify').service('slack') as any;
    ...
    for (const mention of pending) {
      try {
        const result = await (strapi.plugin('analysis').service('ai') as any).analyze(mention);
  ...
  async recluster() {
    ...
    const { exceeded } = await (strapi.plugin('analysis').service('budget') as any).status();
    if (exceeded) {
