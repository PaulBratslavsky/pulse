...
- [ ] **MCP** — `config/server.ts`: `mcp: { enabled: true }` (endpoint `POST /mcp`, Bearer = **scoped Admin API token**; create a **read-only** token limited to mention/topic/event reads for reporting clients). Register custom tools in `pulse-mcp-tools`'s `register()` (must run before `mcp.start()`): `pulse.search-mentions`, `pulse.trend-summary`, `pulse.theme-report` via `strapi.ai.mcp.registerTool(...)`. MCP limitations: no media upload; stateless POST-only
